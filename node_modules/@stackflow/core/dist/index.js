...
function produceEffects(prevOutput, nextOutput) {
  ...
  const output = [];
  const somethingChanged = !(0, import_react_fast_compare.default)(prevOutput, nextOutput);
  if (somethingChanged) {
    output.push({
  ...
  for (let i = 0; i < Math.max(prevOutput.activities.length, nextOutput.activities.length); i += 1) {
    ...
    } else if (prevActivity && nextActivity && prevActivity.id === nextActivity.id && !(0, import_react_fast_compare.default)(
      omit(prevActivity, [
        "isActive",
        "isTop",
        "isRoot",
    ...
    ) && nextActivity.enteredBy.name === "Replaced" && !(0, import_react_fast_compare.default)(prevActivity.enteredBy, nextActivity.enteredBy)) {
      output.push({
        _TAG: "REPLACED",
        activity: nextActivity
      });
